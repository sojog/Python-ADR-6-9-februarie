
# tk - alias conventional
import tkinter as tk
import time


window = tk.Tk()

print(window.winfo_geometry())
print(window.winfo_height(), window.winfo_width())

print(window.winfo_screenheight(), window.winfo_screenwidth())


SCREEN_HEIGHT = window.winfo_screenheight()
SCREEN_WIDTH = window.winfo_screenwidth()


width = height = 800
x = SCREEN_WIDTH // 2 - width // 2
y = SCREEN_HEIGHT // 2 - height // 2
print(window.winfo_geometry())

window.geometry(f"{width}x{height}+{x}+{y}")

label1 = tk.Label(window, text = "Acesta este un label")
label1.pack()
label2 = tk.Label(window, text = "Acesta este un alt label")
label2.pack()


count = 0

def functie_apelata_de_buton():
    global count
    count += 1
    print("Functia a fost apelata de catre buton")
    
    ## OPERATIE COSTISOARE -> networking, db
    time.sleep(5)

    ## UPDATE MAIN THREAD (UI THREAD)
    label1.config(text=f"Apelata de {count} ori")

frame1 = tk.Frame(window)
frame1.pack()


button1 = tk.Button(frame1, text="Acesta este un buton", command=functie_apelata_de_buton)
button1.grid(row=0, column=0)

button2 = tk.Button(frame1, text="Acesta este un alt buton")
button2.grid(row=0, column=1)


frame2 = tk.Frame(window)
frame2.pack()

input1 = tk.Entry(frame2, bg="red")
input1.grid(row=0, column=0)

input2 = tk.Entry(frame2, bg="blue")
input2.grid(row=0, column=1)

def functie_apelata_in_input(event):
    print("Evenimentul:", event)
    print("Noua valoare:", input1.get())

# input1.bind('<Return>', functie_apelata_in_input)
input1.bind('<KeyRelease>', functie_apelata_in_input)

def functie_apelata_print_bind(event):
    print("Evenimentul apelat este:", event, type(event))

button2.bind('<Button-1>', functie_apelata_print_bind)

window.mainloop()